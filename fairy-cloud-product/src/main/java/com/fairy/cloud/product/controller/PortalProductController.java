...
public class PortalProductController {
    ...
    public Result saleProduct(@PathVariable Integer productId, HttpServletRequest request, Principal principal) throws URISyntaxException {
        ...
        ResponseEntity<Result> responseEntity = loadBalance.exchange(url, HttpMethod.POST, entity, Result.class);
...
//        orderFeign.createOrder(omsOrderItemPO);
        ...
        //ribbon调用
        Result rs = loadBalance.getForObject(url, Result.class);

//        url = "http://localhost:8089/stock/deduce?productId=" + productId;
//        Result rs = restTemplate.getForObject(url, Result.class);
        log.info("调用结果:{}", rs);
