...
public class PmsProductServiceImpl implements PmsProductService {
    ...
    private PmsProductPO getProductInfoRedis(Integer id) {
        ...
        //redis 读数据
        Object value = redisTemplate.opsForValue().get(RedisCommons.PRODUCT_DETAIL_CACHE + id);
        if (null != value) {
        ...
        try {
            ...
            if (rs) {
                ...
                redisTemplate.opsForValue().set(RedisCommons.PRODUCT_DETAIL_CACHE + id, productInfo, 3600, TimeUnit.SECONDS);
            } else {
//                //没有那到锁也没有走数据库查询 这样肯定是本身有数据但是拿不到  就算走缓存，但是缓存没有数据
//                Thread.sleep(5 * 1000);
//                //自旋
//                getProductInfo(id);
                value = redisTemplate.opsForValue().get(RedisCommons.PRODUCT_DETAIL_CACHE + id);
                if (null != value) {
                    productInfo = (PmsProductPO) value;
                }
            }
    ...
    private PmsProductPO getProductInfoZk(Long id) {
        PmsProductPO productInfo = null;
        //使用一级本地缓存 jvm
        productInfo = cache.get(RedisCommons.PRODUCT_DETAIL_CACHE + id);
        if (null != productInfo) {
        ...
        if (value != null) {
            ...
            cache.setLocalCache(RedisCommons.PRODUCT_DETAIL_CACHE + id, productInfo);
            return productInfo;
        }
        try {
            //分布式锁
